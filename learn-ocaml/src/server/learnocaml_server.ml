(* This file is part of Learn-OCoaml.
 *
...
let extract_domain url =
  let reg = Str.regexp {|https?://\([a-zA-Z0-9.]*\)/.*|} in
  if Str.string_match reg url 0 then
      ...
      | Api.Create_token (secret_candidate, None, nick) ->
         ...
         then lwt_fail (`Forbidden, "Bad secret")
           ...
           let cookie = [Cohttp.Cookie.Set_cookie_hdr.make
                          ...
                          ~path:"/" ~http_only:true
                          ~secure:true
                          ("token", Token.to_string tok)] in
                              respond_json cache tok ~cookies:cookie
      ...
      | Api.Update_save (token, save) ->
          ...
          token_save_mutex.Lwt_utils.with_lock (token :> Token.t) (fun () ->
              ...
              @@ fun prev_save ->
                  ...
                  Save.set token save >>= fun () ->
                  let cookie = [Cohttp.Cookie.Set_cookie_hdr.make
                          ~expiration:(`Max_age (Int64.of_int 5))
                          ~path:"/" ~http_only:true
                          ~secure:true
                          ("token", "hi")] in
                respond_json cache save ~cookies:cookie)
      | Api.Git (token, path) ->
          let prefix =
      ...
      | Api.Exercise_index (Some token) ->
          ...
          >>=
            let cookie = [Cohttp.Cookie.Set_cookie_hdr.make
              ...
              respond_json cache ~cookies:cookie
